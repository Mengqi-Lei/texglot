...
- **Apple Silicon Mac：**下载 `TeXGlot-1.1.0-macOS-arm64.dmg`，打开后将 TeXGlot 拖入 Applications，再打开应用。
- **Intel Mac：**使用 Release 附件中的 `TeXGlot-1.1.0-macOS-x64.dmg`。
- **Windows x64：**使用 Release 附件中的 `TeXGlot-1.1.0-Windows-x64-Setup.exe`。安装器为当前用户安装，并建立开始菜单和桌面快捷方式，不需要管理员权限。

...
To help validate a platform, run `uv run python scripts/smoke_platform.py --portable-compiler` and attach sanitized results to an issue. This test uses a local model stub, exercises native CLI/PDF generation and does not spend model credits. The [desktop build workflow](https://github.com/Mengqi-Lei/texglot/blob/main/.github/workflows/desktop.yml) builds installers and checks their engines. [Source CI](https://github.com/Mengqi-Lei/texglot/blob/main/.github/workflows/verify.yml) covers Linux, macOS and Windows. Workflow configuration alone does not establish platform support; the table above records the checks completed for this release.

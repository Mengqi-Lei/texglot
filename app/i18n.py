...
MESSAGES = {
    ...
    "请输入有效的 arXiv 论文链接或完整 ID": "Enter a valid arXiv link or a complete identifier",
    "需要完整的 arXiv 版本号，例如 2401.12345v2": "A complete arXiv version is required, for example 2401.12345v2",
    "LaTeX 集成请求需要提供 content_base64": "A LaTeX integration request must include content_base64",
    "LaTeX 源码不是有效的 base64 数据": "The LaTeX source is not valid base64 data",
    "LaTeX 源码为空或超过 80 MB": "The LaTeX source is empty or exceeds 80 MB",
    "LaTeX 源码应为 .tex、.zip、.tar、.tar.gz、.tgz 或 .gz": "LaTeX source must be a .tex, .zip, .tar, .tar.gz, .tgz, or .gz file",
    "当前版本暂不支持 PDF 输入；请使用 arXiv 或 LaTeX 源码": "This version does not support PDF input. Use arXiv or LaTeX source",
    "idempotency_key 格式不正确": "Invalid idempotency_key format",
    "idempotency_key 已用于另一份翻译请求": "This idempotency_key was already used for another translation request",
    "请求体与 Idempotency-Key 不一致": "The request body and Idempotency-Key do not match",
    "任务失败，请在 TeXGlot 中查看处理记录": "The task failed. Open the task log in TeXGlot",
    "LaTeX 编译失败，请在 TeXGlot 中查看处理记录": "LaTeX compilation failed. Open the task log in TeXGlot",
    "模型翻译失败，请在 TeXGlot 中查看处理记录": "Model translation failed. Open the task log in TeXGlot",
    "无法获取 arXiv 源码，请检查论文版本": "Could not fetch the arXiv source. Check the paper version",
